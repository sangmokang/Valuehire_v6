-- Forward-only convergence for environments that already applied
-- 20260901090000. That migration is live in production (recruitment_fee_agreements
-- and invoice_settlement_details exist with its columns), so editing its file would
-- never reach production. Two things move here instead:
--
--   1. The owner policy for authenticated is narrowed to read-only. Writes must go
--      through service_role, which is what the tooling uses.
--   2. The fee-agreement trigger body is re-asserted. 20260901100000 and
--      20260901103000 defined the same 1329-byte body that 20260901090000 already
--      contains, so they were no-ops on a fresh database and were removed; this
--      statement is the forward-only carrier for any database that only ran the
--      earlier text of 20260901090000.
--
-- Rolling back needs no destructive step: re-create the policies with the previous
-- definition.
