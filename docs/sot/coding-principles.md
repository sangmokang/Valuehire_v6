...
- 일반 `WEBHOOK|CREDENTIAL|PRIVATE_KEY` 대입문의 불투명 값 탐지는 12자 이상을 정책 하한으로 삼는다. 6~11자 값은 이 일반 규칙의 비범위이며, 알려진 벤더·세션 형식은 별도 모양 규칙으로 탐지한다. 이는 `keychain`·`PKCS12` 같은 정상 설정값의 상시 오탐을 피하기 위한 확정 경계다.
