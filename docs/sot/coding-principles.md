...
| **P3** | **조용한 실패 금지** | ① 판정은 `PASS / FAIL / NOT_RUN` **3상태**. `NOT_RUN` 이 하나라도 있으면 전체 판정은 PASS 불가 ② bare `except`/`catch` 금지 lint, `?? 기본값`·`catch { return null }`·`\|\| []` 패턴도 린트로 금지하고 명시적 `assertX()` 관문으로 대체. 단 같은 범위에서 `new Map()`으로 선언한 컬렉션의 `.get(...) \|\| []`는 “키 없음”을 빈 컬렉션으로 모델링한 것으로 허용 ③ 모든 핸들러가 status와 함께 DB 기록 |
