...
| **P23** | **초록불은 커밋 SHA 에 귀속된다. 브랜치나 PR 에 귀속되지 않는다** 🆕 | `로컬 HEAD == 원격 브랜치 HEAD == CI 가 실제로 검사한 SHA` 세 값이 모두 같고, 그 SHA 에 붙은 이름 `verify` 실행 **전부**가 completed/success 이며, 작업트리가 깨끗할 때만 **VERIFIED** 다. 알려진 불일치·진행 중·실패가 하나라도 있거나 실행이 0건이면 **UNVERIFIED**, 조회·필터 실패나 오형식·빈 status·빈 conclusion·콜론 둘 이상·알 수 없는 상태가 위치와 무관하게 하나라도 있으면 **NOT_RUN** 이다(모르는 것은 통과가 아니다 · P3). 기계 장치: `scripts/verify/check-verified-sha.sh` 가 브랜치·로컬·원격 SHA와 작업트리 상태 조회의 종료값을 확인하고, SHA 에 붙은 check-run 을 filter=all로 요청해 페이지 전체에서 조회하며, 빈 레코드도 버리지 않은 채 모든 레코드의 구조를 검증한 뒤 상태를 계산한다. `acceptance-verified-sha.sh` 는 SHA 진리표·다중 실행·순서 반례·빈 필드·후속 레코드의 추가 콜론·사유 출력·인자 부족 fail-closed와 실제 gh의 JSON 필터·HTTP 페이지 이동·과거 실패 포함 조회 8건과 실제 조회부의 0건·35건·35건 뒤 오형식·성공 사이 빈 레코드·작업트리 상태 조회 실패·dirty·부분 출력 뒤 조회 실패·필터 실패·브랜치 및 로컬·원격 SHA 부분 출력 뒤 실패까지 52건을 검증한다. **이 스크립트는 완료를 선언하지 않는다 — 상태를 계산해 출력할 뿐이다**(P14 의 일반화: 판정 수치뿐 아니라 판정 자체를 사람·LLM 의 문장에서 뗀다). | PR #31 원격 HEAD 는 929247a, 로컬 HEAD 는 26ad98c, 초록불은 또 다른 옛 커밋 것이었는데 그 상태로 "CI 통과"라고 말할 수 있었다 |
