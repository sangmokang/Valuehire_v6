...
| 5 | Weekly Ops 공용 Skill·fail-closed mutation 게이트 | `bash scripts/acceptance-weekly-ops-skill.sh` — Codex 정본·Claude 어댑터·44개 단위 계약·mutation 14종 |
...
| 21 | 인수 검사 verified-sha | `bash scripts/acceptance-verified-sha.sh` — 초록 판정의 SHA 귀속과 fail-closed |
| 22 | 인수 검사 ci-step-integrity | `bash scripts/acceptance-ci-step-integrity.sh` — 조건부 비활성화·오류 무시 차단 |
