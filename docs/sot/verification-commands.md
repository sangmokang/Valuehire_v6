...
| 4 | HumanSearch G3 포털 상수·locator 경계 | 인라인 — `acceptance-hs-portal-constants.sh` + `-mutations` + `-hardening` + `-hardening2` + `-hardening3` + `-hardening4` (운영 상수·locator 는 contracts/ 한 곳 · P22, exit 0/1/2 3상태. hardening~4 는 V1 적대검증 1~3차 + codeaudit 결함 봉쇄 증명. 검사기가 존재하는 G3 파일 전부의 CI 실행 줄을 요구하고, run:| 아닌 모든 리터럴 스칼라 블록을 실행 칸에서 배제) |
