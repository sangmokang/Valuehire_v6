...
**워크플로 스텝 31개 전부**를 적는다(2026-09-10 hs-1300·hs-1301b·hs-1308·hs-1309 추가·Codex V1 지적으로 `PostgreSQL 서버 준비`·`Invoice 게이트 자가시험`을 별도 행으로 분리해 `- name:` 개수 31과 1:1 로 맞춤)(2026-08-12 V1 D6: 이전 판은 `bash ...` 직접 명령만 적어 인라인 본문 스텝이 목록에서 빠졌고, 운영자가 실제로 무엇이 도는지 잘못 판단할 수 있었다). 아래는 `verify.yml` 의 `- name:` 스텝 순서 그대로다.
...
| 29 | PostgreSQL 서버 준비 | 인라인 — Invoice 런타임 검사용 임시 PostgreSQL 기동(검사가 아니라 준비 스텝) |
| 30 | 인수 검사 invoice | `bash scripts/verify/run-acceptance.sh scripts/acceptance-invoice.sh` — 채용 수수료 계산·기한·계약 변조·Codex/Claude 스킬 동등성 |
| 31 | Invoice 게이트 자가시험 | 게이트 배선 검사 + Python 단위시험 직접 실행 + 임시 PostgreSQL에서 마이그레이션·수수료 동시성·저장/전달 RPC 검증(위조 출력·스텝 무력화 차단) |
