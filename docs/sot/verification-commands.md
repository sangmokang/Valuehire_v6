...
### `​.github/workflows/audit.yml` — 시간-부패 검사 전용 (2026-09-07 신설)

`verify.yml`은 push/PR이 있을 때만 돈다. "억제(suppressions.yaml) 만료일이 지났는가"처럼 **커밋이 없어도 시간이 지나면 저절로 문제가 되는 검사**는 push/PR 트리거만으로는 아무도 못 본다 — 그래서 별도 스케줄 워크플로를 둔다.

| 트리거 | 무엇을 하나 |
|---|---|
| `schedule` (매일 KST 05:00) + `workflow_dispatch` | `scripts/check-suppression-expiry.sh`(억제 만료 — `verify.yml`의 동명 스텝과 **같은 스크립트**를 공유, 로직 이원화 금지), `scripts/check-action-sha-drift.sh`(서드파티 GitHub Action이 가변 태그로 되돌아갔는지) |

- `verify.yml`이 아니므로 이 워크플로는 PR 병합 게이트가 아니다 — 실패해도 PR이 안 막힌다(의도된 설계, 새벽 순찰 역할).
- 실패 시 알림은 GitHub 기본 동작(스케줄 워크플로 실패 시 저장소 계정 이메일)에 맡긴다 — 별도 이슈 자동 생성 등은 이번에 추가하지 않았다.
- `hooks/pre-commit`의 억제 만료 검사는 **다른 입력**(스테이징된 커밋 콘텐츠)을 보는 별도 구현이라 이 스크립트와 통합하지 않았다.
