...
검사  : ⓪ 고정 필수 파일(docs/sot/principles.yaml,
        ...
        ⑧ P3 조용한 실패 문법 판정 — Python은 표준 AST(프로그램 구조를 읽는 표준 기능),
          JS/JSX/TS/TSX는 문자열·주석·템플릿·정규식·JSX 본문과 실행 토큰을 분리하는
          전용 토큰 판정기로 bare except/catch, catch{return null}, ||[]/||=[], ??/??=를 차단한다.
          처리 코드가 있는 바깥 catch도 내부 실행 토큰을 끝까지 순회해 중첩 위반을 차단한다.
          작업트리가 아니라 인덱스 내용을 임시 파일로 만들어 공용 판정기 한 벌을 실행한다.
        ⑨ P3 판정기 또는 뮤테이션 시험이 같은 커밋에서 바뀌면, 인덱스에 들어갈 확정본으로
          뮤테이션 시험의 비재귀 구간을 다시 실행한다. 알려진 위반 차단과 정상 대조군 통과가
          함께 성립하지 않으면 커밋을 막는다.
...
P3의 CI 검사는 P1과 선후 관계가 없는 별도 `p3` 작업에서 실행한다. P1 전체 검사는
미충족 원칙이 있으면 의도적으로 실패하므로 같은 순차 작업 뒤에 두면 P3가 건너뛰어진다.
별도 작업은 P1 성적과 무관하게 P3를 실행하고, P3가 실패하면 그 작업과 전체 서버 판정은
계속 불합격이다.
