...
**정직한 잔여 한계 (현재 정본: `codeaudit-g3-followup-goal-2026-08-15.md` — 이번에 막지 않음):**

| 번호 | 후보 | 저장소 검사만으로 확정하지 못하는 이유 |
|---|---|---|
| 1 | `strategy.max-parallel: 0` | 서버가 0을 어떻게 해석하는지 실제 판정이 필요함 |
| 2 | 작업 수준 `timeout-minutes: 0` | 서버 유효성 판정이 필요함 |
| 3 | 단계 수준 `timeout-minutes: 0` | 서버 유효성 판정이 필요함 |
| 4 | 존재하지 않는 `container` 이미지 | 이미지 가져오기 실패가 작업 실패인지 실행 정지인지 서버 결과가 필요함 |
| 5 | 값이 원격 설정에 달린 `runs-on` 식 | 저장소 밖 변수 값에 따라 실행 위치가 달라짐 |
| 6 | `concurrency` 취소 유발 | 다른 실행의 도착 시점에 따라 취소 여부가 달라짐 |
| 7 | `environment: production` | 원격 승인·보호 설정에 따라 대기 여부가 달라짐 |

→ 과거 이 문서는 1~6 중 `environment`를 뺀 여섯 후보를, verdict9은 `runs-on` 식을 뺀 다른 여섯 후보를 기록했다. 두 목록을 합친 일곱 후보 모두 현재 검사기가 허용함을 임시 복제본 실행으로 확인했으며, 과거 판정서는 당시 증거로 보존하고 이 표를 현재 목록으로 사용한다.

- 검사기의 위협 모델은 "저장소 글자만으로 무력화가 **확정 계산되는** 우회"를 막는 것이다. 위 일곱 후보는 결과가 서버 상태·설정·실행 시점에 달려 저장소 글자만으로 "검사를 건너뛰고도 성공"한다고 확정할 수 없으므로 이번 차단 범위에서 뺐다.
- 서버 검사는 현재 합치기 차단선이 아니다. 2026-08-15 원격 조회에서 main은 보호되지 않았고(`protected=false`), 현재 요금제에서는 보호 규칙과 저장소 규칙 조회가 모두 403으로 거부됐다. 로컬 pre-push도 `git push --no-verify`로 우회할 수 있으므로, 현재 강제력은 사람 규율과 실패 표시까지다. 합치기를 기계적으로 막으려면 요금제를 올리거나 저장소를 공개한 뒤 `verify` 성공을 필수로 지정해야 한다.
