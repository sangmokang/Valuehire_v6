...
현재 PC 관측: Codex CLI 0.154.0, uv 0.11.3. 이 숫자는 강제 설치 버전이 아니다.
...
`uv python install 3.14`로 필요한 Python을 준비한다. uv는 기본적으로 누락된 Python을 자동 설치하지만,
...
이미 받은 ClickUp 카드이며, 매번 프로젝트 생성 승인을 다시 묻지 않는다. 카드 원문과 JD는
인증된 ClickUp 조회로 다시 확보한다. 운영 입력값은 아래 보호 데이터와 같은 경계로 취급한다.

Aside를 본인 환경에 준비한 뒤 공식 제어 경로로 앱 식별·무변경 읽기부터 다시 확인한다.
...
운영 프로젝트 입력, SQLite DB, 보호 감사 원출력은 Git·PR·이메일에 넣지 않는다.
...
|---|---|---|---|
| HS13 결함 6건 | `task/hs-13-stack-20260910` / [#83](https://github.com/sangmokang/Valuehire_v6/pull/83) | 2acc828, 발송 무결성 반례 4개 차단·격리 변이 2개 검출 | 전체 회귀·외부 감사·현재 SHA CI 최종 대조 |
...
| HS02.02 증거 입력 검증 | `task/hs-0202-evidence-validation-20260914` | 3f5ac7f, 잘못된 배열·객체/개인정보 경로 반례 수정 | 재감사·변이 시험·원격 전달 진행 |
| HS05.01 Aside 정책 | `task/hs-0501-aside-policy-20260914` / [#86](https://github.com/sangmokang/Valuehire_v6/pull/86) | 18fc1b4, 주석·중복키 변이 8개 차단·CI 2개 성공 | 전체 외부 V1 미실행; runtime 권한 증명은 별도 |
...
| HS04.01 저장 정책 | `task/hs-0401-storage-policy-contract-20260914` | 235a62d 후보, §12 통합 독립 V2 승인 | 외부 감사·§12 커밋·원격 전달 진행 |
| HS11.03 LinkedIn 저장 개정 | `task/hs-1103-linkedin-storage-contract-20260914` | e9e91e4, 외부 V1·독립 V2 문서 승인 | 원격 전달 진행; §16 runtime 시험 NOT_RUN이므로 Draft |
