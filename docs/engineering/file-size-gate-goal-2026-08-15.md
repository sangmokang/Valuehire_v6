...
이 게이트는 만든 뒤 일곱 라운드의 적대검증을 거쳤다. 각 라운드는 다른 세션(V1=codex, V2=Claude 격리)이 산출물만 받아 새 우회를 찾고, 그 우회를 재현하는 실패 시험(RED)을 먼저 커밋한 뒤 최소 수정(GREEN)으로 닫는 순서로 진행됐다. 판정 원문은 모두 저장소 안 `.claude/private-reviews/`에 보존(경로+내용 지문).
