...
1. **D-1**: 0바이트 증거 3개 + 실제 HEAD SHA + 러너 재실행 5건 체인이 **거부**되는가. 거부 사유가 "체인"이 아니라 **"에이전트가 만들 수 없는 외부 객체 부재"**여야 한다. ⚠️ **2026-08-08 정정**: 2판은 이 조건을 *"'실질성 미달' 또는 '외부 앵커 부재/불일치'"*로 바꿔 적었다. 그런데 §4-3이 정의한 앵커(3사 도메인 응답 `Date` + 응답 해시)는 `curl` 한 번이면 **로그인 없이 만들어진다** — 즉 "에이전트가 만들 수 없는 것"을 요구받고 "만들 수 있는 것"으로 치환했다. **판정 기준 자체를 약화한 것이므로 V1 원문으로 되돌린다.** 이 기준에서 **D-1은 아직 닫히지 않았다.**
...
**최종 상태 (2026-08-08 정정)**: ⚠️ 2판은 여기에 *"V1 결함 10건 전부 반영"*이라고 적었다. **거짓이다.** 같은 문서 §10이 *"지금 시각의 영수증을 만드는 것은 막지 못한다"*고 자백하는데, D-1의 재검증 조건(§9-1)은 바로 그것을 요구했다. 두 문장이 서로를 반박하고 앞의 것이 요약으로 쓰였다.

**실제 상태**: **D-1 미해결** · D-5/D-7/D-8/D-9 부분 · 나머지 5건 반영. 이 판정은 2026-08-08 `/verify` 2단계(품질)·3단계(보안) 리뷰가 각각 FAIL을 내며 확인했고, 그 결함 목록은 3판이 다룬다. **G·V1·V2가 T 기준에서 갈리지 않는다는 2판의 선언도 철회한다** — 갈렸고, V1이 옳았다.
