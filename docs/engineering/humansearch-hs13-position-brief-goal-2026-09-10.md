...
| 8 | RPS 프로젝트·South Korea·Boolean AND/OR 반복 서치 | **부분 채택** | Boolean 검색식 생성은 순수 함수(HS-13.07)로 지금 만든다. 2026-09-14 최신 결정은 RPS 프로젝트가 없으면 생성, 있으면 기존 필터를 업데이트하고 재조회로 중복 생성을 막는 것이다. 그 라이브 조작 계약은 새 정본 `docs/sot/humansearch-rps-project-contract.md` 가 소유한다(이 브랜치에는 아직 미병합 참조). HS-13은 RPS에 옮길 `SearchFilters`·Boolean 산출물과 패킷 검증만 소유하고, 과거 "프로젝트 생성 비범위" 문구로 같은 승인을 다시 요구하지 않는다. |
...
@dataclass(frozen=True) class SearchPacket:   # packet_id == f"{position.clickup_task_id}-{sha256(jd.text)[:8]}" 강제. **조립·역직렬화·저장 공통 경계에서 JD 3종 재검증**: gmail 누락 0·추가 조건 0, linkedin 은 linkedin_omitted_sections 밖 누락 0·추가 조건 0, two_field_jd 는 two_field_sections 로 split_two_field 재계산과 내용 줄 일치(Codex 10차)(Codex 8차: 형식만 맞는 임의 id 는 새 발송 namespace). search_filters.location 은 담을 때의 계약으로 재검증
    ...
    search_filters: SearchFilters; created_on: date; schema_version: int = 1   # HS-13.01b·13.09c·13.09g. packet_id 에 날짜를 넣지 않는다(아래 packet.py), legacy JSON 은 거부
```
...
#   필드1 = 회사 소개(company_intro), 필드2 = section_markers 로 고른 절 + **최상위 마커 절 바로 뒤에 이어지는 괄호 소제목 절(`[…]`)을 흡수**(소제목을 마커로 고르면 그 절 하나만, HS-13.04b —
...
def to_json(packet) / from_json(text)  # 왕복 동일성. schema_version=1 필수, legacy/unknown version 거부. PII는 파일에만, 로그 0
...
def recipients_digest(to: tuple[str, ...], cc: tuple[str, ...]) -> str   # to·cc 각각 정렬, 역할은 분리, 중복 0. SendIntent 수신자 digest 의 표준 입력(D13)
def claim_send(dir, packet_id, channel, attempt, *, at, evidence, recipients_sha256, body_sha256) -> tuple[SendIntent, bool]
...
def mark(dir, packet_id, channel, attempt, state, message_id, at, evidence) -> SendIntent   # SEND_CLAIMED→SENT_UNVERIFIED(message_id 필수) 단방향, transitions append. INTENT→SENT_UNVERIFIED·mark(VERIFIED) 는 거부(청구 없는 발송 표시·readback 없는 검증 표시 차단)
def open_new_attempt(dir, packet_id, channel, *, approval: Approval, at, recipients_sha256, body_sha256) -> tuple[SendIntent, bool]
...
#   uv run --no-sync python -m humansearch.brief verify --packet <path> --sent <readback-receipt.json> --mark-dir <ledger> --message-id <id> --at <iso>
...
#   `--mark-dir` 경로는 구조화 readback 영수증 JSON(`packet_id`·`attempt`·`message_id`·`to`·`cc`·`body`)을 요구한다. 최신 attempt 가 이미 `SENT_UNVERIFIED` 이고 영수증의 attempt/message_id/to-cc/body 가 현재 패킷·장부와 모두 맞을 때만 VERIFIED 전이를 append 한다. `SEND_CLAIMED` 상태나 평문 readback 만으로는 이전 attempt 결과를 재사용해 VERIFIED 로 올릴 수 없다.
...
| D13 | 패킷-발송 digest 결합 | SendIntent 의 `recipients_sha256` 은 `recipients_digest(mail.to, mail.cc)` 만 사용한다(to·cc 내부 정렬, 역할 분리, 중복 거부). `claim_send` 는 현재 intent digest 와 호출 digest 를 대조하고, 같은 저장소의 packet JSON 이 없으면 거부하며 잠금 안에서 현재 패킷 body/to-cc digest 를 다시 계산한다. `verify_and_mark` 는 구조화 readback 영수증 검증 뒤 최신 attempt 가 이미 `SENT_UNVERIFIED` 이고 같은 attempt/message_id/to-cc/body 를 가진 경우에만 현재 패킷 body·recipients digest 와 다시 대조해 VERIFIED 를 만든다. 같은 packet_id 에 intent 가 생기면 다른 패킷 내용으로 저장할 수 없고, packet JSON 은 `schema_version=1` 없이는 로드하지 않는다 | 공급자 멱등키 도입 시 message_id 결합 강화 |
...
공통 인수 명령 꼬리: `cd humansearch && uv run --no-sync ruff check src tests && uv run --no-sync mypy --strict src tests`.

**검사기의 경계(HS-13.00, Codex 4차 중간 지적에 대한 답):** `acceptance-hs-1300.sh` 는 이 문서가 *구조·결합·실존*(카드 26·결정 D1~D13·ID↔파일명·참조 파일이 **이 브랜치 트리**에 실존(형제 브랜치·refs 조회 없음 — CI clean checkout 에서도 같은 판정, Codex 7차)·타입/함수 시그니처·D9 문구·`verification-commands.md` 행의 카드/결정 개수가 실제 루프와 일치)을 갖췄는지만 기계로 판정한다. 양성/음성 서술과 결정 기본값의 **의미 적합성은 정규식으로 판정할 수 없다** — 그것은 Codeaudit(읽기 전용 감사)과 사장님 검토의 몫이며, 이 문서는 그 두 검토를 `## 적대 검증 로그` 에 남긴다. 검사기는 무의미 반복(같은 3자 이상 조각 3회 반복, 같은 단어 반복)만 추가로 거른다.

| WU | 행동 하나 | 인수 명령 (저장소 루트에서 그대로 실행) | 정상 / 반례 | 상태 |
|---|---|---|---|---|
| HS-13.00 | 이 스펙·타입·장부를 저장소에 남긴다 | `bash scripts/verify/run-acceptance.sh scripts/acceptance-hs-1300.sh` (exit 0, `CHECKED: 98`) + `bash scripts/verify/run-acceptance.sh scripts/acceptance-hs-1300-mutations.sh` (`CHECKED: 34`) | 양성: 이 문서 그대로 → 검사기 exit 0 (CHECKED 98). 음성: 문서 부재·WU 행 삭제·§4 catch-all 삭제·WU 셀 비움·D 셀 비움·토큰만 남긴 최소 문서·catch-all 부정어 반전·가짜 명령 경로·무의미 D값·정상/반례 셀 `x`·**id 접미 위장 파일명·정상/반례 셀을 '없음'으로만 채움·D값을 한글 1자+숫자로·IMPLEMENTED 뒤 괄호로 실존 검사 회피·기대출력 자리의 `RESULT=$(id)`·홀수 백틱·LOCAL_COMMITTED(task/…) 괄호 상태·정본 표의 카드 수 14** → 각 exit 1 | IMPLEMENTED |
...
| HS-13.04c | JD 블록 밖 채용 조건 은닉을 거부한다 | `cd humansearch && uv run --no-sync pytest -q tests/test_hs_1304c.py` (`3+ passed`) | 양성: §6 JD 블록 안 원문 조건은 그대로 통과. 음성: 본문 꼬리의 경력·연봉·학력 조건 추가는 패킷 생성 시 거부 | IMPLEMENTED |
...
| HS-13.05b | 팀 메일 입력이 packet-id 줄을 만들 수 없게 한다 | `cd humansearch && uv run --no-sync pytest -q tests/test_hs_1305b.py` (`3+ passed`) | 양성: 러너가 readback 꼬리로 붙이는 packet-id 는 §10 소유. 음성: key_line·sender_name 이 줄바꿈이나 앞공백으로 `packet-id:` 줄을 만들면 조립 거부 | IMPLEMENTED |
...
| HS-13.09g | 패킷 schema·수신자 digest·intent 뒤 저장 불변성을 묶는다 | `cd humansearch && uv run --no-sync pytest -q tests/test_hs_1309g.py` (`3+ passed`) | 양성: schema_version=1 JSON·to/cc 내부 순서가 달라도 같은 digest. 음성: legacy/unknown schema 거부·to와 cc 역할 변경은 다른 digest·중복 주소 거부·intent 이후 같은 packet_id 다른 내용 저장 거부 | IMPLEMENTED |
...
| HS-13.10c | VERIFIED 전이를 readback·digest·message_id 대조와 결합한다 | `cd humansearch && uv run --no-sync pytest -q tests/test_hs_1310c.py` (`2+ passed`) | 양성: CLI `verify --mark-dir --message-id --at` 이 최신 attempt 를 VERIFIED 로 append. 음성: readback 본문·현재 패킷 수신자·message_id 중 하나라도 다르면 VERIFIED 전이 없음 | IMPLEMENTED |
...
| HS-13.10 | 실제 포지션 1건 라이브 | §10 절차 + `cd humansearch && uv run --no-sync python -m humansearch.brief verify --packet <path> --sent <readback-receipt.json> --mark-dir <ledger> --message-id <id> --at <iso>` (exit 0, 출력 `VERIFIED packet_id=ID body_sha256=HEX recipients_sha256=HEX`) | 양성: 2026-09-10 실측 패킷 `86exwz89j-5449bcf5`, Gmail message `1a08971ffd281a48`, RAW MIME text/plain readback 은 당시 초안·발송본과 해시가 일치했다. 다만 이 건은 13.09d/13.10c 이전 legacy artifact 라 현행 코드의 claim/message_id/verify_and_mark 결합을 재검증하지 못한다. **발송 후 발견한 결함**: 필드 2(사람인·잡코리아용)가 `주요업무`·`혜택 및 복지` 본문을 빠뜨렸다 — HS-13.04b 로 수정. 음성: 불일치 → exit 1 `SENT_UNVERIFIED`; 같은 패킷 재실행 → 발송 0. 현행 production 검증은 다음 실제 발송에서 새 절차로만 주장한다 | IMPLEMENTED |
| HS-13.11 | RPS 프로젝트 생성·기존 필터 업데이트 계약과 HS-13 산출물 연결 | `bash scripts/verify/run-acceptance.sh scripts/acceptance-hs-1311-rps-readonly.sh` (`CHECKED: 4` — 프로젝트 readback 1·필터 적용 readback 1·캡차/2FA fixture 즉시 STOP 1·발송·저장 호출 0 정적 1) | 양성: 새 RPS 계약(`docs/sot/humansearch-rps-project-contract.md`, 이 브랜치 미병합 참조)이 프로젝트 생성/기존 필터 업데이트 뒤 readback 으로 중복 생성을 막고, HS-13 패킷의 Boolean/SearchFilters 를 입력으로 받는다. 운영 대상 ClickUp/JD/filter 값은 Git에 기록하지 않는다. 음성: 읽기 adapter 선행이 없거나 캡차/2FA면 즉시 STOP, 쓰기 연결은 상위 RPS 계약만 소유 | BLOCKED(선행 HS-05·HS-11.04~06·새 RPS 계약 병합 전) |
| HS-13.12 | 패킷 → SQLite 이관 | `cd humansearch && uv run --no-sync pytest -q tests/test_hs_1312.py` (`8+ passed`) | 양성: 패킷 1건 → `search_packets`·`candidate_leads` 행 → 재조회 왕복 동일. 음성: 같은 packet_id 2회 → 1행·손상 패킷 거부·PII 열이 암호화 경계(HS-03.03) 밖에 평문 0 | BLOCKED(선행 HS-03.01 저장 스키마·HS-03.03 암호화 경계·HS-04.02 러너 격리 병합 전) |

→ 00 → 01 → (01b, 02, 06, 07 병렬) → 03 → 04 → 05 → 08 → 09 → 10. 11·12는 선행 병합 뒤. 카드 수 = 26(00~12 + 01b·01c·02c·04b·04c·05b·09c·09d·09e·09f·09g·10b·10c).
...
- HS-13.00~09: `NOT_APPLICABLE`(내부 로직). HS-13.10 현행 코드는 새 발송이 `claim_send(True)` → 외부 발송 → `mark(SENT_UNVERIFIED, message_id)` → `verify --mark-dir` 순서를 모두 지난 경우에만 VERIFIED 로 본다. `BUSINESS_USED`는 사장님이 실제 그 메일을 팀에 전달·활용한 뒤에만 쓴다.
- **2026-09-10 라이브 1건은 historical legacy artifact**: 당시 패킷 `86exwz89j-5449bcf5`(번개장터 Product Manager(Core Product)), To 1명(D2), Gmail message id `1a08971ffd281a48`, 본문 sha256 `3c7f360d…881261` 는 정규화 후 초안·발송본·RAW readback 셋이 일치했다. 그러나 이 발송은 13.09d/13.10c 이전 규약(`created=True` → send, 공개 mark 경로)으로 나갔으므로 현행 코드의 claim/message_id/verify_and_mark 결합을 재검증하는 증거로 쓰지 않는다. 현행 증명은 다음 실제 발송부터 §10 순서를 새로 통과해야 한다.
- 비범위: 후보 자동 발송, LinkedIn 로그인 세션 획득, ChatGPT 브라우저 자동화, 사람인·잡코리아 등록 자동화, Supabase 업로드, Discord. RPS 프로젝트 생성·기존 필터 업데이트 자체는 HS-13 비범위가 아니라 새 RPS 계약(`docs/sot/humansearch-rps-project-contract.md`, 이 브랜치 미병합 참조) 소유다.
...
### 2026-09-10 Codex V1 11차 (fresh·read-only, session `01a089c4-a567-7d20-b9e7-e4e63afe3366`, 스택 d469955 대상)

`VERDICT: FAIL` — 높음 3·중간 1(CHECKED NOT_RUN, 선택 회귀 8 passed). 처분(같은 PR, HS-13.04b 확장):
- [높음] 임의 `mail.body` 도 패킷 왕복·VERIFIED → `SearchPacket._check_mail_embeds_jd`: §6 3절 마커(각 1회) 뒤 줄이 jd_packet 3종과 글자 그대로 일치해야 조립·역직렬화 통과. 임의 본문·블록 안 추가/삭제·마커 중복 6종 거부 시험. 모든 시험 fixture 의 메일 본문을 마커 형식으로 갱신(마커는 렌더러가 감싸므로 linkedin_body 자체에는 넣지 않는다).
