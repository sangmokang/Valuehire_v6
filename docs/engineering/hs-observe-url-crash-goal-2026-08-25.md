...
- **위 두 항목은 두 전제 아래에서만 참이다** — ① 출력을 받는 쪽이 파이프를 열어 두고 있을 것
  (닫으면 종료값 120 + stderr 2줄, 아래 참조) ② 계약의 허용 origin·경로·호스트가 ASCII 일 것
  (WU9 이후 계약 검증이 강제하므로 정상 경로에서는 자동으로 성립한다). 종료값 120 이 보이면
  관측기가 아니라 그 실행을 감싼 파이프라인을 먼저 본다.

## 비범위

- CDP 연결·브라우저 기동·포트 발견(D1 소유, `NOT_RUN`)
- `_cdp.py`의 websocket URL 검증
- observe.py의 P11 soft 300줄 초과 해소 — **312줄 → 398줄(+86)**. hard 600은 넘지 않는다.
  분할은 별도 요청으로 남긴다(이 PR에서 파일을 쪼개면 diff가 결함 수정을 가린다).
  ※ 초판에 "~25줄 늘어난다"고 썼는데 실측과 3배 차이가 났다. V2가 잡아 정정했다 — 착수 시점의
  추정을 완료 시점에 다시 재지 않으면 문서가 조용히 거짓말을 한다. **같은 줄이 WU14 이후 또
  낡았다**(391 → 398). 숫자는 인용할 때마다 다시 재는 것 말고는 방법이 없다.
- 재발 원장 파일 신설
...
**독립성 한계 (V2가 잡아 정정).** 위 문단의 "산출물만 전달하고 구현 추론 과정은 전달하지
않았다"는 **2회차에만 참이다.** 3회차 이후 프롬프트에는 직전 회차의 지적을 어떻게 고쳤는지를
내가 요약해 넣었다(예: 6회차 프롬프트 13~17행). 즉 후반 회차의 V1은 완전한 블라인드가 아니다.
그만큼 후반 PASS의 무게는 전반 FAIL보다 가볍게 읽어야 한다.
...
> **갱신 이력**: V2 가 지적한 세 가지(`_valid_origin` 의 `isascii()` 누락, 변조 N2·N7·N21 생존)는
> 이 목록에 남기지 않고 **WU9 에서 닫았다.** 아래는 실제로 남은 것만이다.
...
### WU12 — 무한한 변조군을 만나 상수 대신 계약을 고정했다

V1 이 마지막으로 낸 변조: `for candidate in targets:` → `for candidate in targets[:32]:` 가
모든 시험을 통과한다. 어떤 시험도 타깃을 32개 넘게 주지 않았기 때문이다.

**이 지적은 코드 결함이 아니다.** 그리고 이 모양의 변조는 **원리적으로 무한하다** — 내가 32를
덮으면 64를, 64를 덮으면 128을 고르면 된다. 어떤 유한한 시험 묶음도 "임의 상수로 자르기" 전부를
죽일 수 없다. 그래서 상수를 쫓지 않고 **브라우저 계약 §7 이 요구하는 성질 자체**를 고정했다:

> "제목, 탭 순서, 최근 활성 시각, URL의 부분 일치, 과거 target id, 이전 작업의 탭 번호로 하나를
> 추측하지 않는다."

승인된 탭이 목록 어디에 있어도 같은 결과여야 한다는 뜻이다. Hypothesis 로 앞뒤 위치를 0~80 까지
움직이며, 그리고 승인 탭이 둘 이상일 때 거리를 벌려 가며 검사한다.

```
targets[:32]  → exit=1 | 2 failed
targets[:64]  → exit=1 | 2 failed
targets[:100] → exit=1 | 1 failed
targets[:160] → exit=1 | 1 failed
```

→ 뭘 시켰나: V1 이 지목한 상수와 그보다 큰 상수 셋으로 같은 변조를 걸었다. 뭐가 나왔나: 넷 다
시험이 죽었다. 좋은 소식 — 성질을 고정하면 그 성질을 깨는 모든 상수가 자동으로 잡힌다.

### 적대검증 루프를 여기서 멈추는 이유 (숨기지 않고 적는다)

V1 을 8회 돌렸다. 회차별 발견의 **성격**이 이렇게 옮겨 갔다.

| 회차 | 발견의 성격 |
|---|---|
| 2·3 | **실제 크래시** — 계약된 출력이 사라지고 종료값이 계약 밖으로, 호스트가 오류 메시지에 노출 |
| 4·5 | **실제 크래시** — 제어문자·비ASCII 호스트가 그물을 우회 |
| 6 | 없음(PASS) |
| 7 | **시험 공백** — `except` 원소 하나가 무검증 |
| 8 | **시험 공백** — 도달 불가 주장 반증 + 이벤트 순서 무검증 |
| 9 | **합성 변조** — 임의 상수 절단(무한군) |

→ 표가 말하는 것: 실제 결함은 5회차에서 마르고, 그 뒤로는 시험 공백, 마지막엔 유한한 시험으로
끝낼 수 없는 변조군이 나왔다. 계속 돌리면 계속 무언가 나오겠지만 **그것이 코드를 더 옳게 만들지는
않는다.** 그래서 여기서 멈추고, 남은 것을 사장님 결정 항목으로 올린다.

**이 시점의 상태**: `main()` 을 뚫는 입력 0건(V1 6회차 PASS + 인코더 6종 + 자체 퍼징 1,512건),
무검증 `except` 가지 1개(구분 불가로 증명됨), V1·V2 가 낸 반례 전부 회귀 자산으로 편입.

### WU13 — humanreview 가 **인증 보고의 무방비 불변조건**을 찾았다

병합 직전 `/humanreview` 로 선택 루프·번역기·전송 경계에 **새 변조 10종**을 걸었다.
대조군 99 passed exit 0 기준으로 **5종이 살아남았다.** 그중 둘은 보안 관련이다.

| 변조 | 가드가 사라지면 무슨 일이 나나 | 실측 |
|---|---|---|
| `if not contract_valid: return invalid` 제거 | 화면 구조가 깨졌는데 인증 역할을 주장하는 응답 | `drifted`(exit 2) → **`authenticated`(exit 0)** |
| `if len(roles) != len(matched_roles)` 제거 | 같은 역할을 두 번 주장하는 응답 | `drifted`(exit 2) → **`authenticated`(exit 0)** |
| 읽기 한도(1 MiB) 검사 제거 | 한도를 넘는 목표 목록 응답 | 거부 → **수용** |
| `isinstance(url, str)` → `_origin(str(url))` | 주소 자리에 문자열이 아닌 값 | 후보 제외 → **문자열로 바꿔서 판정** |
| `matching[0]` → `matching[-1]` | (없음) | 등가 |

→ 표가 말하는 것: 앞의 두 줄이 위험하다. 가드가 사라지면 **로그인하지 않은 화면이 인증 성공으로
보고되고 종료값이 0이 된다** — 후속 자동화가 "들어갔다"고 믿는다. L0 계약이 명시한 규칙
(*"`contract_valid` 가 `False`이면 역할 집합과 관계없이 구조가 바뀐 것으로 분류한다"*)인데
그것을 지키는 시험이 하나도 없었다. **코드는 옳았고, 없던 것은 그 옳음을 지키는 것이다.**

이 다섯은 전부 base 부터 있던 코드다. 내가 이번 PR 에서 만든 결함이 아니라, 내 변조 목록이
**내가 손댄 곳만 보고 있었다**는 뜻이다. V2 가 "살아남은 변조 0건은 고른 20종에 한해서만
참"이라고 한 지적이 여기서 다시 확인됐다.

**WU13 이후**: 다섯 중 넷이 죽는다. 남은 하나(`matching[0]` → `matching[-1]`)는 바로 앞 줄에서
`len(matching) != 1` 을 확인하므로 **두 표현이 같은 값을 계산한다** — 어떤 시험도 구분할 수 없는
등가 변조다.

**여기서도 대조군을 빼는 것을 잊을 뻔했다.** 첫 측정에서 모든 변조가 "16건 실패"로 나와 전부
죽은 줄 알았는데, 무변조 대조군도 16건 실패였다(복제본에서 `test_admin_shadow_server.py` 가
소켓을 못 써서 나는 잡음). 대조군을 재고 나서야 5종 생존이 드러났다. 이 run 에서 같은 함정에
두 번째로 걸릴 뻔한 것이다.

### WU14~16 — Codex 적대 검증 (`/codex:rescue`) · `VERDICT: REQUEST_CHANGES`

병합 직전 사장님 지시로 Codex 적대 검증을 한 번 더 돌렸다. 7건을 냈고, 그중 **1건이 진짜
결함**이었다.

| # | 지적 | 재현 | 처분 |
|---|---|---|---|
| 1 | **거부값 충돌**: `_origin()` 의 거부값 `""` 을 허용목록에서 빼지 않아, 허용목록에 빈 문자열이 있으면 파싱조차 못 한 주소가 선택된다 | **재현됨** — 원문 주소를 그대로 든 객체가 반환됨 | **WU14 — 수정 + 시험 6건** |
| 2 | 리뷰 중 HEAD 가 움직였다(`a79a952` → `7e885cb`) | 사실 | 마지막에 SHA 를 다시 고정하고 CI 재실행 |
| 3 | 모든 실패가 `drifted` 로 뭉개진다. 원인 코드(`CAUSE=`)를 남겨라 | 사실 | **오너 결정** — 출력 어휘 신설은 확정되지 않은 결정(§3②) |
| 4 | 12개 WU 중 원 요구는 WU1~2뿐. `0f63ef0` 까지로 분할하라 | 사실 | **오너 결정** — 사장님께 그대로 올림 |
| 5 | 시험 2건이 계약이 아니라 구현 결정을 단언한다 | 사실 | **WU15 — 두 시험을 계약 단언으로 교체** |
| 6 | 퍼징 seed·변조 명세가 저장소에 없어 숫자를 재현할 수 없다 | 사실 | **WU16 — 도구를 저장소 안으로** |
| 7 | 현실적 운영 회귀를 찾지 못했다 | — | 내 실측과 일치 |

→ 표가 말하는 것: 1번이 핵심이다. **거부를 나타내는 값이 허용 판정에 그대로 쓰이고 있었다.**
계약 파일로는 도달할 수 없지만(두 겹이 막는다), 거부값과 허용값이 같은 공간을 쓰는 구조 자체가
결함이다 — 방어 한 겹이 느슨해지는 순간 열린다. WU4(자격증명 누출)와 정확히 같은 모양이다.

**WU15 — 시험이 계약을 단언하게 했다.** Codex 지적대로 둘을 고쳤다.

- `test_three_contract_validators_agree_on_ascii` 는 *"세 검증기가 모두 `isascii()` 를 쓴다"*는
  **내 구현 결정**을 정답으로 박고 있었다. SOT 에는 그런 조항이 없다. → *"계약이 통과시킨 값은
  ASCII stdout 으로도 출력할 수 있어야 한다"*는 **성질**로 교체했다.
- 다중 탭 시험이 `found {count}` 라는 **내부 계수 문구**를 단언했다. 계약이 요구하는 것은
  "거부한다"이지 "몇 개라고 세어 말한다"가 아니다. → `pytest.raises(TargetSelectionError)` 로 교체.

**WU16 — 절단 변조군을 한 번에 닫았다.** Codex 5번이 "승인 탭을 160보다 뒤에 두라"고 했다.
그렇게 했더니 `targets[:250]` 이 살아남았다. 예상대로 **이 변조군은 무한하다.** 상수를 쫓는 대신
반복만 허용하는 목록 객체로 *"선택은 목록을 자르지 않는다"*를 기계로 고정했다.

```
targets[:32]     → 3 failed
targets[:250]    → 1 failed
targets[:1000]   → 1 failed
targets[:100000] → 1 failed
```

→ 뭘 시켰나: 자르는 상수를 100,000까지 키워 가며 변조를 걸었다. 뭐가 나왔나: 전부 시험이
죽었다. 좋은 소식 — 무한한 변조군을 시험 하나로 닫았다. 성질을 고정하면 상수를 쫓지 않아도 된다.

**WU16 — 증거를 저장소 안으로.** Codex 6번이 옳았다: 퍼징 1,512건과 변조 24종의 숫자를 내가
인용했지만 seed·입력목록·runner 가 scratchpad 에만 있어 **남이 재현할 수 없었다.**
`docs/engineering/evidence/hs-observe-url-crash/` 로 옮겼다.

```
fuzz_main.py     → seed=20260825 total=1512 crash=0 bad_exit=0 multiline=0
mutation_sweep.py → 대조군 118 passed exit 0 · 변조 24종 · 생존 1종
```

→ 두 스크립트 모두 **게이트가 아니다**(CI 도 pre-push 도 안 돈다). 문서가 인용한 숫자를 남이
다시 만들어 보게 하는 용도다. `mutation_sweep.py` 는 기대 생존이 그 하나인지까지 스스로
검사하고 다르면 종료값 1 을 낸다.
...
**V1 증거 격리 재현.** verdict.json 의 generator.evidence 7항목을 V2 가 직접 다시 돌렸다.
6항목 재현, 1항목 불일치(내 숫자 표기 오류), 1항목 부분 NOT_RUN.

| # | 내 주장 | V2 실측 | 판정 |
|---|---|---|---|
| 1 | `acceptance-hs-gates.sh` exit 0 · `COLLECTED: 120` | 동일 | 재현 |
| 2 | `verify.sh` exit 0 | 동일 | 재현 |
| 3 | 인수 검사 `PASS=24 FAIL=2` | **`PASS=23 FAIL=2`** | **불일치** |
| 4 | 실소켓 CLI exit 2 · stderr 공백 | 3종 전부 동일 | 재현 |
| 5 | 퍼징 crash=0 | 자체 퍼저 1,500건 `crash=0 bad_exit=0 multiline=0 leak=0` | 재현 |
| 6 | 변조 대조군 56 passed exit 0 | 숫자 일치. 20종 개별 재실행은 NOT_RUN | 부분 재현 |
| 7 | settrace 배선 | 더 긴 경로까지 확인, `_split`·`_readable_port` 실도달 | 재현 |

→ 표가 말하는 것: 내가 낸 증거 대부분은 남이 다시 돌려도 같은 숫자가 나온다. 다만 3번은 내가
`verify.sh` 를 인수 검사 25종에 섞어 세고 라벨은 "인수 검사"라고만 적은 표기 오류였다
(실측: 인수 검사만 `PASS=23 FAIL=2`, `verify.sh` 포함 시 `PASS=24 FAIL=2`).

**V1 명제 축 FAIL — 무엇이 틀렸나.** V1 6회차의 문장
*"I cannot find any configured contract … that escapes as an exception unhandled by `main()`"*
을 V2 가 문자 그대로 반증했다.

- 입력: 계약 `allowed_origins` 에 비ASCII 값 + 승인 origin 과 맞는 탭 1개 + `PYTHONIOENCODING=ascii`
- 기대: 계약 거부 또는 `TAB=-` · 종료값 0/2 · stderr 공백
- 당시 실제: `exit 1` · stdout 빈 문자열 · `observe.py:167` traceback + `UnicodeEncodeError`
- **base `c59bad7` 에서도 동일**하므로 이번 PR 의 회귀가 아니고, 누출 문자열은 계약이 스스로
  선언한 축약 origin 이라 개인정보 침해도 없다.

**근본 원인은 내가 만든 비대칭이었다.** 이 PR 은 `_valid_targets_path` 와
`_is_loopback_address` 에는 `isascii()` 를 넣으면서 `_valid_origin` 에는 넣지 않았다. 회귀가
아니라는 이유로 두고 갈 수도 있었지만, **한 PR 안에서 같은 성격의 검증기 셋을 서로 다르게 두는
것 자체가 결함**이라 WU9 에서 닫았다(코드 1줄 + 시험 6건).

**V2 가 새로 만든 변조 9종 중 3종이 살아남았다.** 이것이 이 판정의 가장 큰 소득이다.

| 변조 | 무엇이 드러났나 | 처분 |
|---|---|---|
| N2 — 전송 `except` 를 `UnicodeEncodeError` 로 좁힘 | `ValueError` **그물의 넓이**를 요구하는 시험이 없었다 | WU9 — 평범한 `ValueError` 를 주입하는 시험 추가 |
| N21 — CDP `except` 를 `JSONDecodeError` 로 좁힘 | 위와 같음 | WU9 — 동일 |
| N7 — `main()` 의 loggable-path 계산을 `frozenset()` 으로 | **`main()` 성공 경로에 닿는 시험이 0건**이었다(기존 셋은 전부 DRIFTED 로 끝난다) | WU9 — 성공 경로 종단 시험 추가 |

→ 표가 말하는 것: "살아남은 변조 0건"은 **내가 고른 20종에 한해서만** 참이었다. 남이 고른 변조로
재보니 세 군데가 비어 있었다. 변조 시험의 값은 목록을 누가 고르느냐에 달려 있다는 실례다.

WU9 이후 N2·N7·N21 은 전부 죽고(각 1 failed), 새로 만든 N22(`_valid_origin` 의 `isascii()`
제거)도 죽는다(3 failed). 대조군 62 passed, exit 0.

**공유 블라인드스팟 — 환경 축.** G(나)의 퍼징 1,512건은 `main()` 을 **같은 프로세스 안에서**
돌려 stdout 을 문자열로 받았다. 그래서 실제 `print()` 의 인코더를 한 번도 통과하지 않았다.
V1 역시 34회 도구 호출 전부 기본 UTF-8 터미널이었다. **둘 다 같은 곳을 안 봤다** — 이것이
교차 검증이 서로를 못 덮는 상관 블라인드스팟의 실례다.

**V2 가 확인한 무위반 항목**: SOT diff 누락 없음(`docs/sot/` 무변경 · `check-docs-sot.sh` OK ·
두 SOT 모두 CLI 출력 형식을 서술하지 않는다) · P3 위반 없음(bare except 0건) · P16 형식 충족
(신규 시험 전부 런타임 import, 소스 텍스트 단언 0건) · P22 위반 없음 · **좁힌 검증이 정상 운영
입력을 막는 사례 반례 0건**(실계약 로드 + 성공 경로 실소켓 관통 `exit 0`,
`TAB=https://hiring.saramin.co.kr/home` 까지 확인 — 나도 V1 도 안 해본 것이다).

**3자 대조**

| 축 | G(나) | V1(codex) | V2(Claude 리셋) | 합의 |
|---|---|---|---|---|
| 지목된 결함이 실재하는가 | 예 | 예 | 예(base 재현) | **합의** |
| 수정이 그 결함을 닫는가 | 예 | 예 | 예 | **합의** |
| 좁힌 검증이 정상 입력을 막는가 | 아니오 | 아니오 | 아니오(성공 경로 실증) | **합의** |
| `main()` 을 뚫는 입력이 남았는가 | 아니오 | 아니오 | **예 → WU9 로 닫음** | 갈렸다가 해소 |
| 살아남은 변조 | 0(20종) | — | **3(9종 신규)** → WU9 로 닫음 | 갈렸다가 해소 |
| 병합 차단 사유 | 없음 | 없음 | 없음 | **합의** |

→ 표가 말하는 것: 갈린 두 줄은 전부 "V2 가 더 깊이 팠고 내가 닫았다"로 끝났다. 남은 갈림은
없다. 다만 V1 후반 회차가 완전한 블라인드가 아니었다는 점(위 §독립성 한계)은 그대로 남는다.
