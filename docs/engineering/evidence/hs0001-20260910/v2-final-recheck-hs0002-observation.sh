#!/usr/bin/env bash
# V2 final recheck copied from V1 final: show why mutant m04 (TOP_KEYS=()) let the two "top ..." tests pass for the wrong reason.
set -u; unset BASH_ENV ENV GIT_DIR GIT_WORK_TREE
D=$(mktemp -d -t v2final-0002); trap 'rm -rf "$D"' EXIT
