#!/usr/bin/env python3
"""V2 final recheck adversarial probes copied from V1 final against the candidate reader/acceptance, run directly (no pytest)
...
base = pathlib.Path(tempfile.mkdtemp(prefix="v2final-probe-"))
...
(ORIG/"artifacts/hs-next-20260910/v2-final-recheck-probes-evidence.json").write_text(json.dumps(ev, ensure_ascii=False, indent=1)+"\n")
shutil.rmtree(base)
