...
건너뛴 것과 한계는 세 가지입니다. 원격 검사는 확인하지 않았고, `ast-grep`는 설치되어 있지 않아 `rg`로 대체했습니다. LSP 진단 도구는 호출했지만 Python을 실제 분석하지 않고 `tsc skipped: no tsconfig found`로 끝났으므로, 실질 타입 근거는 `mypy --strict`입니다. 첫 수동 fixture 재현은 루트 Python에 `pytest`가 없어 실패했고, 같은 재현을 `uv` 환경에서 다시 실행해 성공했습니다.
