...
- **counter-AC (가짜 완료의 모습)**
  ...
  5. 검사 스크립트가 저장소를 오염시킨 뒤 되돌려 통과하면 가짜. 현재 장치는 3축(파일 상태[무시 포함]·HEAD·객체 수) **종료 상태 대조**라 잔존물·HEAD 이동·객체 증가는 잡지만, **중간에 바꿨다 되돌린 변경·같은 개수의 객체 교체는 원리적으로 못 잡는다**(2026-08-12 REV2-D1 실측 반례 4종). 완전한 보증은 원본 쓰기 권한이 없는 격리 실행이 필요하며 후속 AC 후보로 남긴다.
