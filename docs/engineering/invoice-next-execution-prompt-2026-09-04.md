...
> **정정 (2026-09-05)** — 이 문서는 더 이상 실행 지시의 정본이 아니다.
> 이 문서는 "로컬 완료 상태에서 안전하게 통합"을 전제하지만, 2026-09-05 재검토에서
> 저장·검증 결함 9건(D-A~D-I)이 미해결로 확인되어 전제가 깨졌다. 특히
> "2026-09-04 재검증은 단위 테스트 40개 … 모두 PASS" 라는 아래 문장은 사실이지만
> **그 PASS 가 위조 가능했다** — 인수 스크립트 끝에서 결과를 덮어써도 게이트가 통과했다.
> 현재 정본은 `docs/engineering/invoice-defect-closure-goal-2026-09-05.md` 다.
> 이 문서는 역사 기록으로 보존한다(삭제하지 않는다).
