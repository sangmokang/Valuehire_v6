...
## 적대 검증 로그 — 독립 V1(Codex) 재검증, F-1·F-2 확인 + 새 S1 발견·수정 (2026-09-17)

`codex:codex-rescue`로 f5a89fc(F-1·F-2 수정본)에 대한 두 번째 독립 검증(재검증)을 실행했다.
판정: **FAIL**.

- **F-1·F-2 수정 자체는 진짜였다**: 10·40스레드 동시쓰기 각 15회 반복 전부 통과, 스레드 수를
  40→200까지 늘려도 실패율 0%(임계값을 미룬 게 아니라 경쟁 자체를 없앴다는 뜻), 락을 빼고
  옛 재시도값(5회)으로 되돌리면 40스레드 시험이 10회 중 10회 실패로 즉시 재현됨(뮤테이션 킬
  확인). F-2도 www/대소문자 처리를 되돌리면 관련 시험 2개가 즉시 깨짐을 확인.
- **새 결함(S1, 재현 확인)** — `_normalize_url`이 쿼리 문자열을 "추적 파라미터만" 제거하는 게
  아니라 **통째로 버렸다**. 사람인·잡코리아처럼 `?rec_idx=...` 같은 쿼리 문자열로 사람을
  구분하는 채널에서는, `rec_idx`만 다른 서로 다른 두 실제 사람이 정규화 후 같은 값이 되어
  **같은 후보자로 병합**되고, 두 번째 사람의 원본 식별 URL이 조용히 사라짐(첫 번째 사람의
  raw 값이 그대로 반환됨). 직접 재현: `rec_idx=39825930`과 `rec_idx=51002211`을 저장하면
  `candidate_id`가 1=1(병합됨)로 나오고, 두 번째 호출의 `candidate_ref_raw`에 첫 번째 사람의
  URL이 찍힘.
- **수정**: 알려진 추적 파라미터(`trk`, `ref`, `refid`, `fbclid`, `gclid`, `mc_cid`, `mc_eid`,
  `utm_*`)만 화이트리스트로 제거하고, 그 외 쿼리 키(예: `rec_idx`)는 정렬해서 정규화값에
  그대로 남긴다. RED(`test_query_string_identifier_is_preserved_not_treated_as_tracking`,
  수정 전 두 사람이 1=1로 병합) → GREEN(수정 후 서로 다른 후보자로 분리, 원본 URL 각각 보존)
  확인. 추적 파라미터가 실제 식별자와 섞여 있어도 정확히 제거되는지 별도 시험
  (`test_known_tracking_params_are_still_stripped_alongside_a_real_identifier`)으로 확인.
  뮤테이션(추적 파라미터 판정을 무조건 참으로 바꿈)으로 새 시험이 실제로 깨지는지 확인 후 원복.
- **부수 수정(S3, Codex 지적)** — `_write_lock_for`의 락 키가 `str(db_path)`라 경로 표기가
  달라지면(예: 정규화 안 된 `..` 세그먼트) 같은 파일인데 다른 락을 쓸 여지가 있었다.
  `db_path.resolve()`로 키를 만들도록 고쳤다(현재 호출부가 하나뿐이라 실제 위험은 낮았음).

전체 pytest 228건(226+신규 2건) 통과, ruff/mypy strict 통과. Codex가 "다중 프로세스(스레드가
아닌 별도 OS 프로세스) 간 진짜 경합"은 실험 방법의 한계로 확정하지 못했다고 명시했다 — 이
저장소를 부르는 제품 진입점이 아직 없어(NOT_APPLICABLE) 이번 범위에서는 미결로 남긴다.

## 적대 검증 로그 — 독립 V1(Codex) 3차 재검증: PASS (2026-09-17)

`codex:codex-rescue` 3차 검증(90273af 대상). **판정: PASS.** F-1·F-2·쿼리 병합 결함(§위) 모두
반복 실행·뮤테이션으로 재확인했고 회귀 없음. 새로 지적한 것은 병합을 막는 결함이 아니라
비용이 낮은 개선 권고 2건:

- **화이트리스트 밖 추적 파라미터**(`_ga`, `si`, `igshid` 등)가 남아 같은 사람이 여전히 두
  후보자로 쪼개질 수 있음(S2, 설계 위험) — 다만 이 실패 방향(같은 사람이 쪼개짐, 원본은 둘 다
  보존됨)은 이전 결함(서로 다른 사람이 합쳐지고 원본이 사라짐)보다 되돌리기 쉬운 쪽이라
  의도적으로 감수한다.
- **화이트리스트 이름이 미래 채널의 진짜 식별자와 겹칠 위험**(S2) — 코드 주석에 "새 채널
  추가 시 이름 충돌을 확인하라"는 경고를 추가해 반영(`candidate_storage.py:253` 위 주석).
- **파라미터 정렬(`sorted`) 자체를 검증하는 시험 부재**(S3) — 기존 시험은 필터링 후 파라미터가
  1개만 남아 정렬 여부와 무관하게 통과했다. 비추적 파라미터 2개를 포함한 순서-반전 테스트
  (`test_non_tracking_param_order_does_not_affect_dedup`)를 추가하고, `sorted()`를 제거하는
  뮤테이션으로 새 시험이 실제로 잡는지 확인했다.

전체 pytest 229건(228+신규 1건) 통과, ruff/mypy strict 통과. 이 라운드로 3회에 걸친 Codex
독립 검증을 마무리하고 병합 후보로 판단한다.

## 결정 카드 5 — URL 정규화 범위를 승인된 호스트로만 좁힘 (사장님 지시, 2026-09-17)

- **무엇을** — 경로 대소문자 통일과 쿼리 파라미터 정리(추적 파라미터 제거·정렬)를 `linkedin.com`
  한 호스트에만 적용한다. 그 외 모든 호스트는 스킴·호스트 소문자화·`www.` 제거만 하고, 경로와
  쿼리는 원본 그대로 비교한다.
- **왜** — 3라운드에 걸친 Codex 독립 검증에서 같은 함수(`_normalize_url`)가 매번 다른 방향의
  결함을 냈다: 1차(쿼리 미처리)는 같은 사람이 둘로 쪼개짐, 2차(쿼리 전체 삭제)는 서로 다른
  사람이 하나로 합쳐짐. 사장님이 직접 지적하신 대로, 이 패턴은 "정규화 코드 한 줄의 버그"가
  아니라 "무엇을 같다고 볼지"에 대한 계약이 아직 검증되지 않은 채널(사람인·잡코리아)에
  범용 규칙을 적용한 것 자체가 원인이다. 검증되지 않은 채널에는 규칙 자체를 적용하지 않는
  쪽으로 범위를 줄였다.
- **버린 길** — 화이트리스트를 계속 넓혀가며 Codex를 몇 라운드 더 돌리는 방식: 근본 원인(계약
  미검증)을 안 건드리고 증상만 계속 쫓는 것이라 기각했다.
- **대가** — 사람인·잡코리아에서 같은 후보자가 방문 경로가 다른 링크로 재관측되면(예: 이번에
  쓰지 않는 광고 추적 파라미터가 붙은 링크) 별도 후보자 행이 생길 수 있다. 원본은 둘 다
  보존되므로 되돌릴 수 있는 실수이고, 서로 다른 사람이 합쳐지는(원본이 사라지는) 실수보다
  명백히 낫다는 판단이다.
- **되돌리기** — 채널별 실제 URL 계약이 확보되면(제품 진입점 연결 이후) `_APPROVED_CANONICALIZATION_HOSTS`에
  호스트를 추가하는 국소적 변경만으로 확장 가능하다.

RED(승인 호스트 밖에서는 병합되면 안 됨을 검증하는 시험 2건 추가, 승인 목록에 saramin을
몰래 추가하는 뮤테이션으로 실제로 잡히는지 확인) → GREEN. 기존 saramin 기반 추적-파라미터
시험 2건은 그 전제(사람인도 안전하게 정규화 가능)가 새 계약과 충돌해 linkedin.com 기반으로
옮겼다. 전체 pytest 231건, ruff/mypy strict, pre-push G2 게이트(`acceptance-hs-gates.sh`,
`acceptance-hs-gates-mutations.sh`) 전부 통과.

## 적대 검증 로그 — Codex 5차(방법론 제약으로 Claude 직접 실행 대체) (2026-09-17)

사장님이 직접 작성한 5차 프롬프트(기존 발견 재탕 금지, 전체 호출 경로 재추적, 5개 불변조건,
percent-encoding·IPv6·malformed URL 등 미검사 입력공간 요구)로 검증을 시도했다. Codex
자체 실행이 두 차례 모두 실패(첫 에이전트 무응답, 재구동은 네트워크 부재+정책 차단)해
**독립 Codex 검증이 아니라 Claude가 Codex의 미완성 단서를 직접 실행해 확정**한 결과다 —
이 한계를 그대로 기록한다.

발견 3건 중 실제 새 결함은 1건(포트 자리 비숫자 문자열 대소문자 통일, S3)이었다. 나머지
2건(www 제거, 유니코드 NFC 통일)은 이미 이전 라운드에서 명시적으로 검토·결정한 사항의
재지적이었다 — DNS 별칭 관례·유니코드 표준상 "동일 문자의 다른 인코딩"이라는 외부 근거가
있어 "증명 안 된 추측으로 합치는 것"과는 범주가 다르다고 판단해 유지했다. 이 판단은 사장님이
다르게 보시면 뒤집을 수 있는 사안임을 명시한다.

새 결함(포트 자리)은 `_normalized_netloc`을 host/port로 분리해 host만 소문자화·www
제거하도록 수정. RED→GREEN, 뮤테이션 확인. 전체 pytest 235건, ruff/mypy strict 통과.
