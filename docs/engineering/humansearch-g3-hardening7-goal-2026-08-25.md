...
- **모듈 경로 오탐(V1 F8)**: `"package.module"` 같은 점 토큰은 호스트로 오인돼 거부된다. 거부가
  안전한 실패 방향이라 감수하되, 접미사 계약에 일반 단어를 계속 더하는 방식으로 푸는 것은 금지다.
- **결합자 없는 태그 2개 셀렉터(V1 F6 잔여)**: `"dl dd"` 는 놓친다. 이걸 잡으려면 `"a table"`
  같은 영어 문구를 오탐하게 되고(V1 F7), 그 오탐은 결국 억제를 부른다. 의식적으로 미탐을 택했다.
...
- **`git status --porcelain` 기반 자기검사(저장소 무변형)가 두 번 거짓 실패했다.**
  ...
  ② `hardening3` — 실제 `git push` 중 `pre-push` 안에서 1회. 같은 순서(gates→h1→h2→h3)를
     ...
     전부 초록으로 통과했다.
  **원인을 찾았다**(2026-08-25, V1 흡수 작업 중 `git add -A` 가 실패하며 드러났다):

  ```
  error: '.g3-hardening6.u4FuDI/case-1/' does not have a commit checked out
  ```

  `scripts/acceptance-hs-portal-constants-hardening6.sh:53` 이 샌드박스를
  `mktemp -d "$REPO/.g3-hardening6.XXXXXX"` 로 **저장소 안에** 만든다. 추적되지 않고
  `.gitignore` 에도 없어서, hardening6 이 도는 동안 다른 스크립트의
  `git status --porcelain` 에 그 폴더가 나타난다 → `SNAP0 != SNAP1` → "시험이 원본
  저장소를 변형했다"는 거짓 실패. 이 변경이 만든 문제가 아니라 원래 있던 위생 결함이다.

  `.gitignore` 에 `.g3-hardening6.*` 를 넣어 그 창을 닫았다. hardening6 의 동작(같은
  파일시스템에 fixture 를 두는 것)은 건드리지 않았다 — 그건 그 스크립트의 설계다.
...
원문을 지우거나 요약하지 않는다. 11건 중 **8건을 수정으로 흡수**했고 3건은 근거를 붙여
잔여 한계로 남겼다. 각 건의 반례는 `hardening7` 에 표본으로 심어 재발하면 빨개진다.

| ID | 판정 | 내 확인 | 처리 |
|---|---|---|---|
| F1 | 문자열 연결·여러 줄 조립으로 우회 | 재현됨 | **미수정(설계 한계)**. 줄 단위 정규식 게이트 공통의 한계이며 기존 6라운드도 같다. §"잔여 한계"에 명시. 값 조립을 막으려면 언어별 AST 해석이 필요하고 그건 별도 과제다. |
| F2 | 대문자 호스트·대문자 CSS 클래스·전각 마침표 미탐 | 재현됨(`HIRE-PORTAL.ZZUNKNOWN` 수정 전 exit=0) | **수정.** 점 토큰 판정을 대소문자 무시로 넓히고, 전각 마침표(U+FF0E·U+3002·U+FF61·U+02D9)를 판정 전에 보통 마침표로 정규화. 유일한 충돌이던 `"Runtime.evaluate"`(CDP 프로토콜 메서드)는 예외를 파지 않고 `contracts/humansearch/cdp-protocol.json` 으로 옮겼다 — P22 그대로다. |
| F3 | 대문자 확장자(`main.PY`)가 제품 루트 탐지 회피 | 재현됨 | **수정.** 확장자 대조를 `grep -Ei` 로. |
| F4 | 예약 URL 제거가 같은 줄의 실제 URL 까지 삭제 | 재현됨 — `"https://portal.invalid/,https://api.vendor.jobs"` → `""` | **수정(가장 위험했던 건).** 제거 규칙에서 경로 부분을 뺐다. 이제 scheme+호스트까지만 지우므로 뒤의 실제 URL 이 그대로 남아 scheme 규칙에 잡힌다. |
| F5 | 면제 접미사에 실 TLD(py·md·sh·so·zip) 포함 | 재현됨 — 5개 전부 IANA 등재 TLD | **수정.** 다섯 개를 목록에서 제거. 이 저장소 제품 코드가 실제로 쓰는 접미사는 json·css·js·html·txt 뿐이라 손실 없음. 계약 파일에 "앞으로 `conftest.py` 가 걸려도 `py` 를 다시 넣지 마라"를 명시. |
| F6 | 태그 어휘가 좁아 `"dl dd"` 미탐 | 재현됨 | **부분 수정.** 어휘를 40여 개로 넓히고 결합자 형태를 별도 규칙으로 분리(`"dl > dd"` 는 이제 잡힌다). 결합자 없는 태그 2개(`"dl dd"`)는 여전히 놓친다 — F7 과 정면으로 상충하기 때문이다(아래). |
| F7 | 태그 규칙이 평범한 영어 문구를 오탐 | 재현됨 — `"a table"`·`"button label"` 수정 전 둘 다 차단 | **수정.** 이제 (가) 결합자가 있거나 (나) 태그 3개 이상일 때만 잡는다. F6 과의 상충은 의식적 선택이다: 모든 제품 문자열에서 영어 문구를 오탐하는 게이트는 결국 억제를 부르고, 그게 구멍보다 나쁘다. |
| F8 | 점 토큰이 모듈 경로(`"package.module"`)를 오탐 | 재현됨 | **미수정(설계상 감수).** 거부가 안전한 실패 방향이고, 정당한 값은 `contracts/` 로 옮기는 게 이 게이트의 정해진 처리다. 다만 접미사 목록에 `module` 같은 일반 단어를 계속 더하면 게이트가 녹으므로, 그 방향은 계약 파일에서 금지해 뒀다. |
...
`hardening7` 44개 사례 전부 통과(blocked 27, allowed 8, notrun 8, wiring 1).
같은 시험을 수정 전(`e819c33`) 계약에 걸면 첫 V1 반례에서 멈춘다:

```
FAIL: hardening7 [V1 F2 대문자 호스트] exit=0 (기대: 정확히 1)
```

수정 전/후 직접 대조(제품 계약 ERE):

```
MESSAGE = "a table"        수정전=1(오탐)  → 수정후=0
MESSAGE = "button label"   수정전=1(오탐)  → 수정후=0
SEL = "dl > dd"            수정전=0(미탐)  → 수정후=1
```
