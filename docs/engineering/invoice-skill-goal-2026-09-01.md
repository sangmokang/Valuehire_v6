# [ARCHIVED] Invoice 스킬 목표·검증 기록 — 2026-09-01

> 상태: 역사 기록. 2026-09-02에 `docs/sot/invoice.md`와
> `docs/sot/invoice-storage.md`가 이 문서의 운영 결정을 대체했다. 아래의 “기본 20%”,
> `UNCONFIRMED`, `send_enabled=false`, 메일 미발송 결론은 당시 검수 단계 기록이며 현재
> 구현 규칙이 아니다. 현재는 고객사·포지션·적용일별 Supabase 계약만 수수료 정본이고,
> 세금 정책은 `NOT_APPLICABLE`, 발송 기능은 승인 게이트 아래 활성 상태다.
> 이 파일은 현재 정본·현재 테스트 계약·구현 지시의 일부가 아니다. 아래 단언은
> 당시 의사결정과 검증 결과를 재현하기 위한 기록으로만 읽는다.
...
- 당시 사용자 예시 두 건은 결정 연봉의 정확히 20%였다. 이 판단은 2026-09-02 폐기됐으며 운영 기본값으로 사용할 수 없다.
...
## 당시 T 계약 기록: 현재 합격 조건으로 사용 금지

당시 T는 이 문서, `docs/sot/invoice.md`, `contracts/invoice/invoice-v1.json`의
합집합이었다. 현재 T에는 이 문서가 포함되지 않으며, 현재 규칙은
`docs/sot/invoice.md`와 `docs/sot/invoice-storage.md`만 따른다.

### AC-1 — 수수료 계산

- 대체된 EARS 단언: **When** 유효한 결정 연봉을 입력하면, 시스템은 입력된 수수료 금액을 받지 않고 고객사·포지션·적용일에 일치하는 Supabase 계약 수수료율을 Decimal 연산으로 적용해야 한다. 일치 계약이 없거나 둘 이상이면 중단한다.
- 검증 명령: `python3 -m unittest discover -s tools/invoice/tests -v`
- 기대값: fixture의 각 Supabase 수수료 계약을 적용했을 때 60,000,000원은 12,000,000원, 40,000,000원은 8,000,000원이며 전체 시험이 0건이 아닌 상태로 성공한다. 이 숫자는 기본 수수료율을 뜻하지 않는다.
...
- 검증 명령: `python3 -m unittest discover -s tools/invoice/tests -v`
- 기대값: HTML 의미 구조 시험이 필수 항목을 모두 찾고, 검수용일 때 부가가치세 비대상 문구와 DRAFT 표식을 찾는다.
...
- 당시 EARS 단언: **While** 검수본, 계약과 다른 세금 정책, 당시의 `send_enabled=false`, 시각 검증 실패, Gmail 연결 부재 또는 최종 발송 요약 미승인 중 하나라도 있는 경우, 시스템은 Gmail 발송 도구를 호출하지 않아야 한다. 현재 발송 규칙으로 재사용하지 않는다.
...
- 당시 권한 경계: V2는 파일 수정·PDF 재생성·Gmail 호출을 하지 않았다. 당시 세금 정책 `UNCONFIRMED`, 당시 `send_enabled=false`, 당시 사용자 최종 승인 부재 때문에 그 검수 실행의 최종 발행·메일은 `BLOCKED`였다. 이는 현재 상태 판정이 아니다.
