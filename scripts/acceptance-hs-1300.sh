...
wu_row_ok() {
  ...
  printf '%s' "$cmd" | $G -Eq "tests/test_hs_13${id}\.py( |\`|$)|scripts/acceptance-hs-13${id}(-[a-z]+)*\.sh( |\`|$)|python -m humansearch\.brief" || return 1
...
for k in 'position_count: int' 'extra_lines: tuple' 'def extract_block' 'from_attempt: int'; do
