...
# owner/repo@<40자 SHA> 형태만 통과시킨다. 로컬 액션(./path)·Docker 액션(docker://...)은
# 버전 고정 개념이 다르므로 대상에서 제외한다.
#
...
while IFS= read -r line; do
  n=$((n+1))
  if ! printf '%s' "$line" | grep -qE '@[0-9a-f]{40}$'; then
    echo "FAIL: 가변 참조로 고정되지 않은 액션: $line"
    fail=1
  fi
done < <(grep -rhoE 'uses:[[:space:]]*[^./#[:space:]][^[:space:]]*/[^[:space:]]+@[^[:space:]#]+' "$target" --include='*.yml' --include='*.yaml' 2>/dev/null | sed -E 's/^uses:[[:space:]]*//')
