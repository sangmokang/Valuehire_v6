...
for required in "$SOURCE" "$LEDGER" "$SELF" "$PRE_PUSH" "$WORKFLOW"; do
...
pre_push_text = File.read(pre_push_file)
active_pre_push = File.readlines(pre_push_file, chomp: true).map(&:strip)
...
unless pre_push_text.include?("-name 'acceptance-*.sh'")
