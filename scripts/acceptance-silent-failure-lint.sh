#!/usr/bin/env bash
# acceptance-silent-failure-lint.sh — P3 "조용한 실패 금지"의 문법 판정 절반을 강제한다.
#
# 계약:
...
def collect_files(arguments: list[str]) -> list[str]:
    ...
    result = subprocess.run(
        ["git", "ls-files", "-z", "--", "*.py", "*.js", "*.jsx", "*.ts", "*.tsx"],
...
def main() -> int:
    ...
    files = [path for path in candidates if Path(path).suffix.lower() in TARGET_SUFFIXES and Path(path).is_file()]
    if not files:
