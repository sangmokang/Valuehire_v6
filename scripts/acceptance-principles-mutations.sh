#!/usr/bin/env bash
# Strict 원칙 계약의 실패 기대 반례 29개와 정상·경계 대조군 5개를 격리 사본에서 실행한다.
set -uo pipefail
...
BASE="$TMP/base"
mkdir -p "$BASE/docs/sot" "$BASE/scripts" "$BASE/hooks" "$BASE/.github/workflows"
...
checked=0
CASE=""
WEAK_OR_TRUE=$(printf '%s%s' '|' '| true')
WEAK_CONTINUE=$(printf '%s%s' 'continue-on' '-error: true')
...
new_case c10a
ruby -e 'p=ARGV[0]; weak=ARGV[1]; s=File.read(p).sub("run: bash scripts/acceptance-principles-check.sh", "run: bash scripts/acceptance-principles-check.sh #{weak}"); File.write(p,s)' "$CASE/.github/workflows/verify.yml" "$WEAK_OR_TRUE"
expect_principles "C10-A" "CI에 ${WEAK_OR_TRUE} 삽입" 1 FAIL

new_case c10b
ruby -e 'p=ARGV[0]; weak=ARGV[1]; s=File.read(p).sub("      - name: Strict 원칙 정본·장부·배선 검사\n        run:", "      - name: Strict 원칙 정본·장부·배선 검사\n        #{weak}\n        run:"); File.write(p,s)' "$CASE/.github/workflows/verify.yml" "$WEAK_CONTINUE"
...
new_case c10e
ruby -e 'p=ARGV[0]; weak=ARGV[1]; s=File.read(p).sub("bash scripts/acceptance-principles-check.sh", "bash scripts/acceptance-principles-check.sh #{weak}"); File.write(p,s)' "$CASE/hooks/pre-push" "$WEAK_OR_TRUE"
expect_principles "C10-E" "pre-push에 ${WEAK_OR_TRUE} 삽입" 1 FAIL
