...
# 값 하한 경계 (2026-09-05 오너 확정 정책). 위 카나리는 36자라 하한을 20 으로 올려도
# 초록이라 경계를 지키지 못했다. 정확히 12자인 값을 잡는지 못 박는다 — 하한을 12보다
# 올리면 여기서 빨개진다. 11자가 안 잡히는 것은 확정된 대가이므로 시험으로 굳히지
# 않는다(나중에 탐지를 넓히면 그 시험이 오히려 개선을 막는다).
must_catch "키워드 규칙 값 하한 경계(정확히 12자)" "${K_CR}_MATERIAL=Ab1234567890"
